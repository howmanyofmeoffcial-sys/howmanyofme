...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://howmanyofme.co/blog/${article.slug}` },
  };
}
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container py-8 md:py-12">
        ...
        <article className="max-w-3xl mx-auto">
          <div className="mb-10">
            <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-primary/10 text-primary mb-4">{catLabel}</span>
          ...
          <div className="mt-12 p-6 rounded-xl bg-secondary/50 border border-border">
            ...
            <div className="flex flex-wrap gap-2">
              <Link href="/tools/popularity-checker" className="px-3 py-1.5 text-sm rounded-lg bg-card border border-border hover:bg-primary/5 transition-colors">Name Popularity Checker</Link>
              <Link href="/tools/random-name" className="px-3 py-1.5 text-sm rounded-lg bg-card border border-border hover:bg-primary/5 transition-colors">Random Name Generator</Link>
              <Link href="/tools/baby-names" className="px-3 py-1.5 text-sm rounded-lg bg-card border border-border hover:bg-primary/5 transition-colors">Baby Name Ideas</Link>
              <Link href="/names/a" className="px-3 py-1.5 text-sm rounded-lg bg-card border border-border hover:bg-primary/5 transition-colors">Browse Names A–Z</Link>
            </div>
          </div>

          <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {prev && (
              <Link href={`/blog/${prev.slug}`} className="group p-4 rounded-xl border border-border hover:bg-secondary/50 transition-colors">
            ...
            {next && (
              <Link href={`/blog/${next.slug}`} className="group p-4 rounded-xl border border-border hover:bg-secondary/50 transition-colors sm:text-right sm:ml-auto">
          ...
          <RelatedPosts currentSlug={article.slug} tags={getTagsForSlug(article.slug)} count={12} />
        </article>
      </main>
      <SiteFooter />
    </div>
