...
/**
 * Reusable main-tool embed.
 * Goal: give crawlers a strong, contextual link to /name/* from every page,
 * and convert readers into users (engagement signal → indexing).
 */
const ToolCTA = ({
  variant = "full",
  ...
  const submit = (e: React.FormEvent) => {
    ...
    navigate(`/name/${encodeURIComponent(n)}`);
  };
        ...
        <form onSubmit={submit} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full h-11 rounded-lg border-2 border-border bg-background pl-9 pr-3 text-sm focus:outline-none focus:border-primary"
            />
          </div>
          <button
            type="submit"
            className="h-11 px-4 rounded-lg bg-primary text-primary-foreground text-sm font-semibold inline-flex items-center gap-1.5 hover:opacity-90 transition-opacity"
          >
  ...
  return (
    <section
      aria-label="Name popularity checker"
      className="my-10 md:my-14 p-6 md:p-8 rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/5 via-card to-accent/5"
    >
        ...
        <form onSubmit={submit} className="relative max-w-lg mx-auto">
          <div className="relative">
            ...
            <input
              ...
              aria-label="First name"
              className={`w-full h-14 rounded-xl border-2 bg-background pl-12 pr-36 text-base focus:outline-none transition-all ${
                focused ? "border-primary ring-4 ring-primary/15" : "border-border"
              }`}
            />
            <button
              type="submit"
              className="absolute right-1.5 top-1/2 -translate-y-1/2 h-11 px-5 rounded-lg bg-primary text-primary-foreground font-semibold text-sm inline-flex items-center gap-1.5 hover:opacity-90 transition-opacity"
            >
