  ...
  return (
      <main className="container py-12 max-w-3xl">
        ...
        <p className="text-sm text-muted-foreground mb-8">
          ...
          <Link href="/tools/unique-name-generator" className="text-primary hover:underline">Unique Name Generator</Link>. Want random suggestions?{" "}
          <Link href="/tools/random-name" className="text-primary hover:underline">Random Name Generator</Link>.
        </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-12">
          {names.map((n) => (
            <Link key={n} href={`/name/${n}`} className="p-3 rounded-lg border border-border bg-card hover:bg-secondary/50 text-center text-sm font-medium transition-colors">
              {n}
            </Link>
          ))}
        </div>
        ...
        <section className="mb-12 p-6 rounded-xl border border-primary/20 bg-primary/5">
          ...
          <p className="text-muted-foreground">
            The <strong>Baby Name Browser</strong> is a free A–Z directory of 100,000+ real recorded baby names. Click any letter to see all names beginning with it, filter by gender, then tap any name for its full popularity statistics, decade trend, meaning and origin. Every name is sourced from official birth registries from the <a href="https://www.ssa.gov/oact/babynames/" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">US Social Security Administration</a> and <a href="https://www.ons.gov.uk/peoplepopulationandcommunity/birthsdeathsandmarriages/livebirths" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">UK ONS</a> — no AI-invented strings. Use this tool if you're exploring options, or head home to <Link href="/" className="text-primary hover:underline">find out how many people have your name</Link>.
          </p>
        </section>
