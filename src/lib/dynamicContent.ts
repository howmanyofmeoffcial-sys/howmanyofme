...
export function buildSimilarFaqs(ctx: IntroCtx): DynamicFaq[] {
  ...
  const rest = pool.length - 1; // 6
  const pick = (n: number) => pool[(Math.abs(n) % rest) + 1];
  const rotated = [pool[0], pick(seed), pick(seed >> 2), pick(seed >> 4)].filter(Boolean) as DynamicFaq[];
