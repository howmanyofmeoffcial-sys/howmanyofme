import { inferCountriesFromName } from "@/lib/nameOriginCountryMap";
...
export function getNameData(name: string) {
  ...
  const originList = [
    "English", "Latin", "Greek", "Hebrew", "Germanic", "Celtic",
    "Arabic", "Sanskrit", "Hindi", "Filipino", "Scandinavian",
    "Slavic", "Japanese", "Turkish", "Persian", "Chinese",
    "Korean", "French", "Spanish", "Irish",
  ];
  const origin = originList[Math.abs(hash) % originList.length];
  const inferredCountries = inferCountriesFromName(normalized, origin);
  const regions: Record<string, number> = {};
  inferredCountries.forEach(({ country, count: c }) => {
    regions[country] = c;
  });
  ...
  return {
    ...
    rank: Math.max(1, Math.min(count > 100000 ? (Math.abs(hash) % 500) + 1 : (Math.abs(hash) % 50000) + 500, 99999)),
    regions,
    decade_popularity: {
      "1940s": (Math.abs(hash) % 40) + 10,
      "1950s": (Math.abs(hash) % 45) + 10,
      "1960s": (Math.abs(hash) % 50) + 15,
      "1970s": (Math.abs(hash) % 55) + 15,
      "1980s": (Math.abs(hash) % 60) + 20,
      "1990s": (Math.abs(hash) % 65) + 20,
      "2000s": (Math.abs(hash) % 70) + 15,
      "2010s": (Math.abs(hash) % 60) + 15,
      "2020s": (Math.abs(hash) % 50) + 10,
    },
    origin,
