...
import { blogArticles, blogCategories, type BlogArticle } from "@/data/blogData";
...
const BlogIndex = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title="Baby Name Blog — Trends, Guides & Insights | HowManyOfMe"
        ...
        canonical="https://howmanyofme.site/blog"
      />
      <SiteHeader />
      <main className="container py-12">
        <div className="max-w-3xl mb-10">
          ...
          <p className="text-lg text-muted-foreground leading-relaxed">
            Data-driven articles, guides, and insights about baby names — from popularity trends and regional data to naming guides and expert analysis.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${active === "all" ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-primary/10"}`}
          >
          ...
          {blogCategories.map(cat => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${active === cat.id ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-primary/10"}`}
              >
        ...
        {/* Featured article */}
        {active === "all" && (
          <Link to={`/blog/${blogArticles[0].slug}`} className="group block mb-10 rounded-2xl border border-border bg-card overflow-hidden hover:shadow-lg transition-shadow">
            <div className="p-8 md:p-10">
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-4 ${categoryColors[blogArticles[0].category]}`}>
                Featured
              </span>
              <h2 className="font-display text-2xl md:text-3xl font-bold mb-3 group-hover:text-primary transition-colors">{blogArticles[0].title}</h2>
              <p className="text-muted-foreground leading-relaxed mb-4 max-w-2xl">{blogArticles[0].description}</p>
              <div className="flex items-center gap-4 text-sm text-muted-foreground">
                <span className="flex items-center gap-1"><Clock className="h-4 w-4" />{blogArticles[0].readTime} min read</span>
                <span>{new Date(blogArticles[0].date).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</span>
              </div>
            </div>
          </Link>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(active === "all" ? filtered.slice(1) : filtered).map(article => (
            <Link key={article.slug} to={`/blog/${article.slug}`} className="group flex flex-col p-6 rounded-xl border border-border bg-card hover:shadow-md transition-all">
