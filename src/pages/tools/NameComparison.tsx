...
const NameComparison = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SEOHead
        title="Name Comparison Tool — Compare Two Baby Names Side by Side"
        description="Free name comparison tool. See which of two names is more popular, with side-by-side rank, bearer count and decade-by-decade trend bars."
        canonical="https://howmanyofme.co/tools/name-comparison"
        jsonLd={jsonLd}
      />
      <SiteHeader />
            ...
            <div className="grid grid-cols-2 gap-4">
              {[results.a, results.b].map((r, i) => (
                <div key={i} className="rounded-xl border border-border bg-card p-5 text-center">
                  <Link to={`/name/${r.name}`} className="font-display text-2xl font-bold hover:text-primary transition-colors">
                    {r.name}
                  </Link>
