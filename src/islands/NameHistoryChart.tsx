...
export default function NameHistoryChart({
  ...
  const hasSexBreakdown = history.some((h) => (h.male ?? 0) > 0 || (h.female ?? 0) > 0);
  ...
  return (
    <div className="p-5 md:p-6 rounded-2xl border border-border bg-card shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <h3 className="font-display text-lg font-bold text-foreground">
            U.S. Birth Registrations for {name} (1880–2024)
          </h3>
          <p className="text-xs text-muted-foreground">
            Peak Era: <strong className="text-foreground">{peakYear}</strong> (~{peakBirths.toLocaleString()} recorded births)
          </p>
        </div>
        {hasSexBreakdown && (
      ...
      <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span>Includes births with ≥ 5 annual occurrences</span>
      </div>
    </div>
  );
}
